...
export async function createExercise(
  ...
  const title = exerciseData.title || `Test Exercise ${Date.now()}`;
  ...
  const exerciseExists = await exercisesPage.hasExerciseWithTitle(finalData.title);
  if (!exerciseExists) {
    throw new Error(`Exercise "${finalData.title}" was not created successfully`);
  }
