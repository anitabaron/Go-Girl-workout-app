...
- Stan: `LoginFormState` (ViewModel) - `{ email: string; password: string }`
...
### 8.4 Wysłanie formularza (submit)
...
### 8.5 Kliknięcie "Nie pamiętasz hasła?"
...
### 8.6 Kliknięcie "Zarejestruj się"
...
### 8.7 Automatyczne ustawienie focus
...
5. Utworzenie `src/components/auth/login/login-button.tsx` (Client Component - przycisk "Zaloguj się")
6. Utworzenie `src/components/auth/login/login-links.tsx` (Client Component - linki pomocnicze)
7. Utworzenie `src/components/auth/login/validation-errors.tsx` (Client Component - błędy walidacji) lub użycie istniejącego komponentu
