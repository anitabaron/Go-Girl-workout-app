...
<template>
    <section class="bg-goPink pt-6 pb-6">
        <section class="px-4">
            <div class="container-xl lg:container m-auto">
                ...
                <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <OneFullWorkout
                        v-for="workout in state.workouts.slice(0, limit || state.workouts.length)"
                        :key="workout._id"
                        :workout="workout"
                    />
                </div>
        ...
        <section v-if="showButton" class="m-auto max-w-lg mt-5 px-6">
            <RouterLink to="/workouts" class="block bg-goMidRed text-white text-center py-4 px-6 rounded-xl hover:bg-goRed"
