...
const formattedTime = computed(() => {
    const total = isRunning.value || isPaused.value ? timeLeft.value : minutes.value * 60 + seconds.value;
    ...
    <div class="flex flex-col items-center justify-center min-h-screen bg-white p-4">
        ...
        >
            <svg class="absolute top-0 left-0 w-full h-full" viewBox="0 0 100 100">
                <!-- Background -->
                <circle cx="50" cy="50" r="45" stroke="#ffbdc8" stroke-width="9" fill="none" />
                <!-- Progress -->
                <circle
                    v-if="!isCountUp"
                    cx="50"
                ...
                <div class="flex justify-center space-x-6 text-xl">
                    <div class="flex flex-col items-center space-y-1">
                        ...
                        <div class="flex space-x-2">
                            <button @click="adjustMinutes(1)" class="px-3 py-1 bg-gray-100 rounded hover:bg-pink-200">+</button>
                            <button @click="adjustMinutes(-1)" class="px-3 py-1 bg-gray-100 rounded hover:bg-pink-200">-</button>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col items-center space-y-1">
                        <span>Seconds</span>
                        <div class="flex space-x-2">
                            <button @click="adjustSeconds(1)" class="px-3 py-1 bg-gray-100 rounded hover:bg-pink-200">+</button>
                            <button @click="adjustSeconds(-1)" class="px-3 py-1 bg-gray-100 rounded hover:bg-pink-200">-</button>
                        </div>
                    </div>
                </div>
