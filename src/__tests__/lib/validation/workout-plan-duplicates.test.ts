...
describe("scope/single duplicate validation", () => {
  ...
  it("does not block form save on temporary scope/single order collision after reorder", () => {
    const scopeId = "123e4567-e89b-12d3-a456-426614174000";
    ...
    const errors = validateWorkoutPlanFormBusinessRules([
      ...
      {
        exercise_id: "333e4567-e89b-12d3-a456-426614174000",
        section_type: "Main Workout",
        section_order: 1,
        scope_id: scopeId,
        in_scope_nr: 2,
    ...
    expect(
      errors.some(
        (e) =>
          e.includes("W slocie") ||
          e.includes("Duplikat kolejności w sekcji"),
      ),
    ).toBe(false);
  });
});
